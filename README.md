...
托盘菜单中的“编辑菜单”打开 WPF 菜单树管理器，可在 `RunAny.ini` 与 `RunAny2.ini` 之间切换。左侧仅显示分类和菜单项名称；固定操作栏、右键菜单和快捷键均支持置顶、置底、上下移动、提升一级、降为子级、移动到指定分类、删除、撤销与重做。快捷键包括 `Alt+↑/↓/←/→`、`Ctrl+Home/End`、`Ctrl+Z/Y` 和 `Delete`。拖拽仅从节点手柄开始，释放前显示插入线、目标分类高亮和半透明节点预览，并支持边缘自动滚动、悬停展开分类及 `Esc` 取消。将“添加分类”“添加菜单项”“添加普通分隔线（`|`）”或“添加层级复位线（`-`）”拖入菜单树仍可快速新增；右键分类后添加层级复位线，会把复位线放在该分类之后的同一层，使后续菜单项重新回到当前菜单层级。右侧只编辑名称、命令或内容、全局热键和管理员运行；已有 DSL 的其他高级标记会保持不变。保存时保留原有 GBK/UTF 编码、注释、空行及未修改的 DSL 行。托盘菜单不再提供“显示 Everything”和“编辑菜单文件”，新增“关于”窗口显示当前版本、程序与配置目录以及 GitHub 项目地址。
