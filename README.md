...
当前版本：[v0.1.1](https://github.com/wonly211/RASharp/releases/tag/v0.1.1)
...
- [自包含单文件版](https://github.com/wonly211/RASharp/releases/download/v0.1.1/RASharp-v0.1.1-win-x64-self-contained.exe)：无需安装 .NET，直接运行。
- [框架依赖单文件版](https://github.com/wonly211/RASharp/releases/download/v0.1.1/RASharp-v0.1.1-win-x64-framework-dependent.exe)：需要安装 .NET 10 Desktop Runtime x64。
- [SHA-256 校验文件](https://github.com/wonly211/RASharp/releases/download/v0.1.1/SHA256SUMS.txt)。
...
`RASHARP_MENU_HOTKEY` 使用 RunAny/AHK 风格修饰符：`^` Ctrl、`!` Alt、`+` Shift、`#` Win。未设置时使用反引号键。

对于 `Win+S`、`Win+X` 等 Windows Shell 保留组合，RASharp 在 `RegisterHotKey` 失败后会自动改用低级键盘钩子，拦截原系统动作并执行 RASharp 热键。安全桌面组合（例如 `Ctrl+Alt+Delete`）无法由普通桌面应用覆盖。
...
菜单图标以 PNG 持久化到 RASharp 程序目录：主菜单位于 `Cache\RunIcon\MenuIcon`，菜单2位于 `Cache\RunIcon\MenuIcon2`。菜单加载时优先读取缓存，未命中才调用 Windows `SHGetFileInfo`。设置页“图标”中可清空全部缓存并重新提取，也可从 PNG、ICO、JPG、BMP、EXE、DLL、普通文件或文件夹为指定菜单项设置覆盖图标。选择 EXE 或 DLL 时会枚举其中的全部图标资源，以缩略图和资源索引供人工选择。
...
RASharp 每 24 小时最多检查一次稳定版更新，不会自动切换到 Everything 1.5 Alpha。Everything 自身的登录启动和更新通知会关闭，由 RASharp 统一启动、显示和升级；设置页可以配置独立的 Everything 显示热键，默认为 `Ctrl+Alt+F`。
