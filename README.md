...
托盘菜单中的“编辑菜单”打开 WPF 菜单树管理器，可在 `RunAny.ini` 与 `RunAny2.ini` 之间切换。左侧仅显示分类和菜单项名称；将“添加分类”“添加菜单项”“添加分隔线”拖入菜单树即可新增，现有节点只能从左侧拖拽手柄开始拖动，并需短暂停留和超过安全距离后才会调整位置或父子层级。右键菜单还可添加或删除分类、应用和分隔线，以及置顶、置底、上移、下移。右侧只编辑名称、命令或内容、全局热键和管理员运行；已有 DSL 的其他高级标记会保持不变。保存时保留原有 GBK/UTF 编码、注释、空行及未修改的 DSL 行。托盘菜单不再提供“显示 Everything”和“编辑菜单文件”，新增“关于”窗口显示当前版本、程序与配置目录以及 GitHub 项目地址。
