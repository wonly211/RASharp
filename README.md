...
设置保存在 `%APPDATA%\RASharp\settings.json`，保存后会立即重新载入菜单和输入服务。
...
如果没有指定配置目录，程序使用 `%APPDATA%\RASharp`，并在首次启动时生成最小示例配置。
...
dotnet publish src/RASharp.App/RASharp.App.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=true -o artifacts/publish/win-x64
```
...
- `%A_ScriptDir%`、常用 Windows/环境变量、日期时间变量、`%s` 和 `%getZz%`。
...
运行日志和 Everything 缓存位于 `%APPDATA%\RASharp`。
