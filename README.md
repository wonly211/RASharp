...
dotnet publish src/RASharp.App/RASharp.App.csproj -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -o artifacts/publish/win-x64
```

发行包使用便携目录而非单文件捆绑。WPF/.NET 的原生组件在单文件模式下会于启动前解压到 `%TEMP%\.net`，不符合 RASharp 的严格便携存储要求。
