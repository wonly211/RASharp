...
托盘菜单中的“编辑菜单”打开 WPF 菜单树管理器，可在 `RunAny.ini` 与 `RunAny2.ini` 之间切换。左侧仅显示分类和菜单项名称；将“添加分类”“添加菜单项”“添加分隔线”拖入菜单树即可新增，拖动现有节点可排序或调整父子层级。右侧只编辑名称、命令或内容、全局热键和管理员运行；已有 DSL 的其他高级标记会保持不变。保存时保留原有 GBK/UTF 编码、注释、空行及未修改的 DSL 行。“编辑菜单文件”则使用系统默认文本编辑器直接打开当前 `RunAny.ini`。托盘菜单不再提供“显示 Everything”。
