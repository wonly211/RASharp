...
当前版本：[v0.1.4](https://github.com/wonly211/RASharp/releases/tag/v0.1.4)

- 读取 `RunAny.ini` 和可选的 `RunAny2.ini` 树形菜单 DSL。
- 通过 Everything SDK 解析无路径程序、处理同名结果并持久化缓存。
- 一键 Everything 搜索当前选中文字、多选文件或剪贴板内容。
...
- [自包含单文件版](https://github.com/wonly211/RASharp/releases/download/v0.1.4/RASharp-v0.1.4-win-x64-self-contained.exe)：无需安装 .NET，直接运行。
- [框架依赖单文件版](https://github.com/wonly211/RASharp/releases/download/v0.1.4/RASharp-v0.1.4-win-x64-framework-dependent.exe)：需要安装 .NET 10 Desktop Runtime x64。
- [SHA-256 校验文件](https://github.com/wonly211/RASharp/releases/download/v0.1.4/SHA256SUMS.txt)。
...
热键格式使用 `^` 表示 Ctrl、`!` 表示 Alt、`+` 表示 Shift、`#` 表示 Win。主菜单默认使用反引号键；菜单2、设置和一键 Everything 搜索热键默认为空。
...
- 主菜单、菜单2和打开设置页面的全局热键。
- 启用、停用、清空并重建无路径程序缓存。
- 启用、停用 RASharp 托管的 Everything，设置一键搜索热键、文件扩展名和文件夹搜索行为，自动升级并检测连接状态。
...
RASharp 每 24 小时最多检查一次稳定版更新，不会自动切换到 Everything 1.5 Alpha。Everything 自身的登录启动和更新通知会关闭，由 RASharp 统一启动、显示和升级；设置页可以配置独立的一键 Everything 搜索热键，默认留空。按下热键时优先搜索当前选中文字或多选文件，没有选区则使用剪贴板内容；没有搜索内容时切换 Everything 窗口的激活或最小化。选中文件时可选择是否保留扩展名，选中文件夹时可选择搜索文件夹内部。
...
托盘菜单中的“编辑菜单”打开 WPF 菜单树管理器，可在 `RunAny.ini` 与 `RunAny2.ini` 之间切换。左侧仅显示分类和菜单项名称；固定操作栏、右键菜单和快捷键均支持置顶、置底、上下移动、提升一级、降为子级、移动到指定分类、删除、撤销与重做。快捷键包括 `Alt+↑/↓/←/→`、`Ctrl+Home/End`、`Ctrl+Z/Y` 和 `Delete`。拖拽仅从节点手柄开始，释放前显示插入线、目标分类高亮和半透明节点预览，并支持边缘自动滚动、悬停展开分类及 `Esc` 取消。将“添加分类”“添加菜单项”“添加分隔线”拖入菜单树仍可快速新增。右侧只编辑名称、命令或内容、全局热键和管理员运行；已有 DSL 的其他高级标记会保持不变。保存时保留原有 GBK/UTF 编码、注释、空行及未修改的 DSL 行。托盘菜单不再提供“显示 Everything”和“编辑菜单文件”，新增“关于”窗口显示当前版本、程序与配置目录以及 GitHub 项目地址。
